...
As all variables are standardized, the covariance matrix $\boldsymbol{\Sigma}$ reduces to the correlation matrix $\boldsymbol{\rho}$
$$
\boldsymbol{\rho} = 
\begin{bmatrix}
1 \\
...
\end{bmatrix}.
$$
Due to standardization, the residuals $\epsilon$ are normally distributed,
$$
...
$$
\beta = \sqrt{\frac{R^2}{\sum diag(r \cdot r^T) + \sum r r^T \boldsymbol{\rho}}},
$$
